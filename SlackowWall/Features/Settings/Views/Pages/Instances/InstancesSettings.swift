...
struct InstancesSettings: View {
    ...
    var body: some View {
        SettingsPageView(title: "Instances") {
            ...
            SettingsCardView {
                VStack {
                    SettingsToggleView(title: "Force Aspect Ratio (16:9)", description: "Forces the instances to use this aspect ratio which is useful when using stretched instances.", option: $settings.forceAspectRatio)

                    Divider()
                        .padding(.bottom, 4)
                    
                    SettingsToggleView(title: "Show Instance Numbers", option: $settings.showInstanceNumbers)
                }
            }
            ...
            SettingsCardView {
                VStack {
                    ...
                    Form {
                        HStack {
                            Group {
                                TextField("X", value: $settings.moveXOffset, format: .number.grouping(.never))
                                    .textFieldStyle(.roundedBorder)
                                
                                TextField("Y", value: $settings.moveYOffset, format: .number.grouping(.never))
                                    .textFieldStyle(.roundedBorder)
                                
                                TextField("W", value: $settings.setWidth, format: .number.grouping(.never))
                                    .textFieldStyle(.roundedBorder)
                                
                                TextField("H", value: $settings.setHeight, format: .number.grouping(.never))
                                    .textFieldStyle(.roundedBorder)
                            }
                            .frame(width: 80)
