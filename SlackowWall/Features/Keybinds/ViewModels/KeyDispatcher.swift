...
class KeyDispatcher {
    ...
    static func sendKey(key: KeyCode, pid: pid_t) {
        ...
        guard let kspd = CGEvent(keyboardEventSource: src, virtualKey: key, keyDown: true) else {
            return
        }
        guard let kspu = CGEvent(keyboardEventSource: src, virtualKey: key, keyDown: false) else {
            return
        }
